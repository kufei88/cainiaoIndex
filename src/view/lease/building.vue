...
<div>
    ...
    <!-- 新增办公楼按钮 -->
    <Button icon="md-add" @click="isAddNewData = true" style="float:left">新增</Button>
    <!-- Excel导入按钮 -->
    <Upload action="" :before-upload="handleBeforeUpload" accept=".xls, .xlsx" style="float:left">
        <Button icon="ios-cloud-upload-outline" :loading="uploadLoading" @click="handleUploadFile">上传文件</Button>
    </Upload>
    <!-- Excel导出按钮 -->
    <Button icon="md-download" :loading="exportLoading" @click="exportExcel" style="float:left">导出文件</Button>
    <!-- Excel模板下载按钮 -->
    <Button icon="md-download" :loading="exportLoading" @click="exportExcelModel" style="float:left">模板下载</Button>
    <div style="clear:both"></div>
    <!-- 自定义查询模块 -->

    <!-- 新增办公楼弹窗 -->
    <Modal :closable="false" v-model="isAddNewData" title="新增办公楼信息填写">

        <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="100">

            <FormItem label="办公楼号" prop="buildingNumber">
                <Input clearable v-model="formValidate.buildingNumber" />
            </FormItem>

            <FormItem label="办公楼名称" prop="buildingName">
                <Input clearable v-model="formValidate.buildingName" />
            </FormItem>
    ...
    <Table border :columns="dataColumns" :data="pageData" ref="table">
    ...
    <!-- 分页功能 -->
    <Page :total="dataCount" @on-change="changePage" show-sizer :current="pageCurrent" @on-page-size-change="changePageNumber" :page-size-opts="[10,20,50,100]" align="center" />
</div>
</template>
    ...
    data() {
        ...
        var isBuildingNumber = function (rule, value, callback) {
            ...
            for (var key in thisVue._data.historyData) {
                if (value == thisVue._data.historyData[key].buildingNumber) {
                    isDataSame = true;
                }
            }
